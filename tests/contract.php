<?php
$path=__DIR__.'/contracts/verifact-api-3.4-openapi.json';$failures=[];
$spec=json_decode((string)file_get_contents($path),true);
if(!is_array($spec)){$failures[]='OpenAPI contract must be valid JSON';}
if(($spec['info']['version']??'')!=='3.4.0'){$failures[]='API contract version must be 3.4.0';}
...
if($failures){fwrite(STDERR,implode(PHP_EOL,$failures).PHP_EOL);exit(1);}echo "VeriFact WordPress/API 3.4 contract checks passed.\n";